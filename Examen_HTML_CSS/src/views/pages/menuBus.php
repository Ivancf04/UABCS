...
<?php
        ...
        $totalProducts = $_GET['total'];
        $totalPages = ceil($totalProducts / 10);
        $currentPage = $_GET['page'];
        $products = buscarProducts($texto, $currentPage, $limit=10);
?>
            <div id="caja_opciones">
                ...
                <a href="../pages/carrito.php?idProducto=<?=$currentPage?>&userId=<?= $idUser?>&carrito=<?=$idCarrito?>"><button><img src=  "<?=ASSETS_URL?>/imgs/carrito-de-compras.png" alt="img"></button></a>
            </div>
    ...
    <main id="contenedor_principal">
        <div id="home">
            <button onclick="window.location.href='./menu.php?page=1&userId=<?=$idUser?>&carrito=<?=$idCarrito?>'"><img src="<?=ASSETS_URL?>/imgs/home.png" alt="img"></button>
        </div>
        
        <div id="contenedor_productos">
            <?php foreach($products as $product):?>
            <a style="color:inherit; text-decoration:none" href="detalles.php?idProducto=<?=$product['id']?>&userId=<?= $idUser?>&carrito=<?=$idCarrito?>">
        ...
        <div class="nav">
            <?php if ($currentPage > 1): ?>
                <button id="izquierdo" class="navegar" onclick="window.location.href='?page=<?= $currentPage - 1 ?>&userId=<?=$idUser?>&carrito=<?=$idCarrito?>&texto=<?=$texto?>&total=<?=$totalProducts?>'"><</button>
            <?php endif; ?>
            <?php if ($currentPage < $totalPages): ?>
                <button id="derecha" class="navegar" onclick="window.location.href='?page=<?= $currentPage + 1 ?>&userId=<?=$idUser?>&carrito=<?=$idCarrito?>&texto=<?=$texto?>&total=<?=$totalProducts?>'">></a></button>
            <?php endif; ?>
        </div>
